...
@RestController
public abstract class BaseController<ENTITY extends NamedEntity<ID>,
        SERVICE extends NamedServiceInterface<ENTITY, ID>, ID>
        implements NamedControllerInterface<ENTITY, ID> {
    ...
    public ResponseEntity<?> update(@PathVariable ID id, @RequestBody ENTITY entity) {
        ...
        if(productOptional.isPresent()) {
            return getResponseBuilder().save(getService().update(id, productOptional.get()));
        } else {
